...
async def main(options: argparse.Namespace) -> None:
    ...
    blueiris_only = detector_config.getboolean("blueiris-only", False)
    cams = []
    i = 0
    while "cam%d" % i in config.sections():
        cams.append(
            Camera(config["cam%d" % i], excludes.get(config["cam%d" % i]["name"], {}), mqtt_client, blueiris_url, blueiris_only)
        )
        i += 1
